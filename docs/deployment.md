...
| `DATABASE_URL` | Supabase → Connect → Transaction pooler | Prisma 런타임용 pooled PostgreSQL URL. `pgbouncer=true` 포함 |

## 새 DB 구성 — Vercel + Supabase PostgreSQL

새 제품의 데이터베이스는 Docker나 자체 서버를 운영하지 않고, **기존 Supabase 프로젝트의 PostgreSQL**을 사용한다. 레거시 Supabase Auth/API 코드와 테이블은 새 흐름에 재사용하지 않지만, 관리형 PostgreSQL 자체는 Prisma와 표준 연결 문자열로 사용할 수 있다.

1. Supabase Dashboard → 프로젝트 선택 → **Connect**에서 Transaction pooler URL과 Direct connection URL을 각각 복사한다.
2. Transaction pooler URL(포트 6543)에 `pgbouncer=true`를 붙여 Vercel의 Production·Preview·Development `DATABASE_URL`로 저장한다. Vercel 서버리스 함수에는 이 pooled URL을 사용한다.
3. Direct connection URL은 로컬 쉘에서만 `SUPABASE_DIRECT_URL`로 설정하고, 아래처럼 migration/seed를 실행한다. 이 값은 레포·Vercel 런타임·클라이언트에 넣지 않는다.
...
4. Vercel에서 재배포한 뒤 앱은 pooled `DATABASE_URL`만 사용한다.

기존 Supabase 프로젝트의 현재 플랜·휴면 정책·사용량 한도만 대시보드에서 확인한다. MVP 단계에서는 DB를 상시 실행하는 VM·Docker 볼륨·백업 크론보다 이 구성이 운영비와 관리 부담이 적다.
