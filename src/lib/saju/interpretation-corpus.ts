...
function sourceFragment(id: string, domains: ReportDomain[], conditions: CorpusCondition[], grounds: string[], source: ScriptSource, strength: CorpusFragment["strength"], guidance: string): CorpusFragment {
  return { id, status: "approved", version: CORPUS_VERSION, domains, direction: "neutral", strength, conditions, grounds, mechanism: source.mechanism, translation: source.translation, concreteScene: source.concreteScene, guidance };
}

const dayMasterFragments = Object.entries(DAY_MASTER_SCRIPT).map(([dayMaster, source]) => sourceFragment(`F-DAY-${dayMaster}`, ["essence"], [{ field: "dayMaster", equals: dayMaster }], ["day-master"], source, 5, "성향을 단정하지 말고, 이 장면이 왜 자주 생기는지 친구처럼 알아봐 준다."));
const tenGodFragments = Object.entries(TEN_GOD_GROUP_SCRIPT).map(([group, source]) => sourceFragment(`F-GROUP-${group}`, ["yearly", "career"], [{ field: "dominantTenGodGroup", equals: group }], ["dominant-ten-god-group"], source, 3, "현실 장면은 하나만 골라 쓰고, 돈·직장 결과를 단정하지 않는다."));
const annualFragments = Object.entries(annualSource).map(([god, source]) => sourceFragment(`F-ANNUAL-${god}`, ["yearly"], [{ field: "annualTenGod", equals: god }], ["annual-ten-god"], source, 4, "올해의 바깥 환경으로만 풀고, 취업·합격·수입을 예언하지 않는다."));
const interactionFragments = Object.entries(INTERACTION_SCRIPT).flatMap(([key, source]) => {
  if (key === "화극금" || key === "수극화") return [sourceFragment(`F-INTERACTION-${key}`, ["mental"], [{ field: "mentalInteraction", equals: key }], ["mental-interaction"], source, 4, "건강 진단이나 사건 예고 없이, 감정과 행동의 패턴으로만 풀어낸다.")];
  return [sourceFragment(`F-MONTHLY-${key}`, ["relation"], [{ field: "monthlyRelation", equals: key === "충" ? "clash" : "combination" }], ["monthly-relation"], source, 4, "관계의 종료·이별·사고를 단정하지 말고, 해당 월의 대화와 일정 리듬으로만 쓴다.")];
});

const careerFragments: CorpusFragment[] = [
  { id: "F-CAREER-ASSET-PEER", status: "approved", version: CORPUS_VERSION, domains: ["career"], direction: "caution", strength: 5, conditions: [{ field: "careerPattern", equals: "asset-with-peer" }], grounds: ["da-yun-ten-god", "career-pattern"], mechanism: "재성의 실속 감각 위에 비겁의 경쟁·분산 기운이 함께 놓인다. 큰 요행이나 사람과 엮인 한 방을 좇으면 자원과 에너지가 새기 쉬움.", translation: "큰돈을 노리기보다 알짜배기 실속과 결과물을 챙겨야 하는 10년의 판.", concreteScene: "변동 큰 투자로 한 방을 노리기보다 월급·연봉 협상·자격증·포트폴리오처럼 손에 남는 결과물을 차곡차곡 쌓는 장면.", guidance: "투자 손익을 단정하지 말고, 한 방과 실속의 선택 기준으로만 직설적으로 설명한다." },
  { id: "F-CAREER-ASSET", status: "approved", version: CORPUS_VERSION, domains: ["career"], direction: "opportunity", strength: 4, conditions: [{ field: "careerPattern", equals: "asset" }], grounds: ["da-yun-ten-god", "career-pattern"], mechanism: "재성이 대운의 앞에 있어 들어오는 자원보다 무엇을 남기고 어디에 쓸지를 현실적으로 보게 만든다.", translation: "한 번의 화제성보다 지속 가능한 결과물을 챙기는 10년의 판.", concreteScene: "예산·일정·포트폴리오처럼 시간이 지나도 남는 결과물을 하나씩 쌓는 장면.", guidance: "수입 증가를 약속하지 않고, 자원을 운영하는 장면에 집중한다." },
  { id: "F-CAREER-RESPONSIBILITY", status: "approved", version: CORPUS_VERSION, domains: ["career"], direction: "mixed", strength: 4, conditions: [{ field: "careerPattern", equals: "responsibility" }], grounds: ["da-yun-ten-god", "career-pattern"], mechanism: "대운의 흐름에 책임과 평가의 기운이 함께 얹혀 맡은 일의 범위와 결과물이 분명할수록 힘이 실린다.", translation: "애매하게 넓히기보다, 내 이름으로 끝까지 남길 일을 고르는 10년의 판.", concreteScene: "정리된 제안서 하나와 끝까지 마무리한 프로젝트 하나를 남기는 장면.", guidance: "승진·이직을 확정하지 말고, 맡은 범위와 결과물의 관계로만 쓴다." },
  { id: "F-CAREER-GENERAL", status: "approved", version: CORPUS_VERSION, domains: ["career"], direction: "neutral", strength: 2, conditions: [{ field: "careerPattern", equals: "general" }], grounds: ["da-yun-ten-god"], mechanism: "10년의 대운 배경 위에 올해 세운이 겹쳐, 당장의 반응보다 오래 남길 일의 방식을 보게 한다.", translation: "올해의 선택을 10년짜리 일의 판과 연결해 보는 시기.", concreteScene: "당장 멋져 보이는 일보다 다음에도 꺼내 쓸 수 있는 결과물 하나를 고르는 장면.", guidance: "대운을 운명처럼 말하지 않고, 긴 흐름의 배경으로만 쓴다." },
];

const defaultFragments: CorpusFragment[] = [
  { id: "F-RELATION-DEFAULT", status: "approved", version: CORPUS_VERSION, domains: ["relation"], direction: "neutral", strength: 2, conditions: [{ field: "monthlyRelation", equals: "none" }], grounds: ["annual-ten-god"], mechanism: "뚜렷한 충·합이 확인되지 않을 때는 올해 환경의 역할과 압박이 관계 리듬에 간접적으로 닿을 수 있다.", translation: "관계를 운명처럼 해석하기보다, 바쁜 날의 말투와 답장 속도가 달라질 수 있는 정도로 보는 흐름.", concreteScene: "피곤한 날 단톡방 답장을 미뤘다가, 마음이 정리된 뒤 짧고 정확하게 다시 답하는 장면.", guidance: "이별·새 인연을 예언하지 않고, 대화의 속도와 경계만 현실적으로 다룬다." },
  { id: "F-MENTAL-DEFAULT", status: "approved", version: CORPUS_VERSION, domains: ["mental"], direction: "neutral", strength: 2, conditions: [{ field: "mentalInteraction", equals: "null" }], grounds: ["annual-ten-god"], mechanism: "올해 들어오는 역할과 내 기준이 맞물리면 생각을 오래 끌기 쉬울 수 있다.", translation: "할 일을 다 끝낸 뒤에도 혼자 반성회를 길게 열지 않도록 보는 흐름.", concreteScene: "보낸 메시지를 계속 다시 읽고 싶어질 때, 다음 날 한 번만 확인하고 화면을 닫는 장면.", guidance: "건강 진단 없이, 작고 구체적인 멈춤 장치 하나를 제안한다." },
];

const summaryFragment: CorpusFragment = { id: "F-SUMMARY-RESULT", status: "approved", version: CORPUS_VERSION, domains: ["summary"], direction: "neutral", strength: 3, conditions: [], grounds: ["da-yun-ten-god", "annual-ten-god"], mechanism: "대운은 긴 흐름의 배경이고 세운은 올해의 환경이다. 둘을 합쳐 당장 무슨 일이 일어난다고 결론내리지는 않는다.", translation: "더 많은 일을 벌이는 것보다 이미 가진 준비를 손에 남는 결과물로 바꾸는 쪽에 힘이 실리는 해.", concreteScene: "이번 주 할 일 중 남에게 보여 줄 하나, 돈이나 시간을 아낄 하나, 미뤄 둔 대화 하나를 골라 끝내는 장면.", guidance: "한 줄 응원으로 마무리하되 운명적 약속을 하지 않는다." };
